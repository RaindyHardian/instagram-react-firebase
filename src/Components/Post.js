...
const Post = ({
  ...
  const [likeSVG, setLikeSVG] = useState(
    <svg
  ...
  const postComment = () => {
    ...
    db.collection(`posts/${postId}/comments`)
      ...
      .then(commentRef => {
        db.collection(`users/${postUser_id}/activity`).add({
          user_id: db.doc("users/" + loggedInUser_id),
          type: "comment",
          postId: postId,
          timestamp: firebase.firestore.FieldValue.serverTimestamp(),
          comment_id: db.doc(`posts/${postId}/comments/${commentRef.id}`)
        });
        setInputComment("");
    ...
    <div className="post">
      <div className="post__header">
        <div className="post__profile">
          ...
          <h3>
            <Link
              to={"/profile/" + postUser_id}
              className="post__headerUsername"
            >
              {username}
            </Link>
          </h3>
      ...
      />
      <div
        style={{ display: imgLoading ? "flex" : "none" }}
        className="post__image post__imageLoading"
      >
        Loading...
      </div>
      ...
      <h4
        className="post__likeText post__text font-gray"
        onClick={() => setModalLikeOpen(true)}
      >
      ...
      {comments.length !== 0 ? (
        <>
          {comments.length > 1 ? (
            <p
              className="post__seeComment"
              onClick={() => setModalCommentsOpen(true)}
            >
          ...
          <p className="post__text font-gray">
            <Link
              to={"/profile/" + comments[comments.length - 1].user_id}
          ...
          {inputComment === "" ? (
            ...
            <>
              {postLoading ? (
                ...
                <button className="post__commentButton" onClick={postComment}>
                  Post
                </button>
              )}
            </>
      ...
      <p className="post__timestamp">
        {moment(timestamp.toDate().toString()).fromNow()}
      </p>
      ...
      >
        <div style={modalStyle} className="post__commentM">
          <div className="post__commentTitle font-gray font-bold">Comments</div>
          <div className="post_commentListContainer">
            ...
            <div className="post__modalCaption">
              <div className="post__modalCommentItem post__text font-gray">
                ...
                <div>
                  ...
                  {" " + caption}
                  <p className="post__modalTimestamp">
                    {moment(timestamp.toDate().toString()).fromNow()}
                  </p>
                </div>
              </div>
            ...
            {comments.map(({ id, comment, username, user_id, photoUrl }) => (
              <div
                className="post__modalCommentItem post__text font-gray"
                key={id}
              >
                <Avatar
                  className="post__avatar"
                ...
                <div>
                  ...
                  <p className="post__modalTimestamp">
                    {comment.timestamp
                      ? moment(comment.timestamp.toDate().toString()).fromNow()
                      : null}
                  </p>
                </div>
              </div>
      ...
      {/* View Likes Modal */}
      <Modal open={modalLikeOpen} onClose={() => setModalLikeOpen(false)}>
        <div style={modalStyle} className="post__like">
          <div className="post__likeTitle font-gray font-bold">Likes</div>
