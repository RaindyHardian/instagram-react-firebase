...
const SinglePost = props => {
  ...
  const postComment = () => {
    ...
    db.collection(`posts/${id}/comments`)
      ...
      .then(commentRef => {
        db.collection(`users/${post.user_id}/activity`).add({
          user_id: db.doc("users/" + props.user.uid),
          type: "comment",
          postId: post.id,
          timestamp: firebase.firestore.FieldValue.serverTimestamp(),
          comment_id: db.doc(`posts/${post.id}/comments/${commentRef.id}`)
        });
        setInputComment("");
            ...
            db.collection(`posts/${id}/likes`)
              ...
              .then(() => {
                const userRef = firebase
                  ...
                  .collection("users")
                  .doc(props.user.uid);
                db.collection(`users/${post.user_id}/activity`)
  ...
  return (
    <div className="singlePost">
      ...
      {/* image */}
      {!isLoading ? (
        <img className="post__image" src={post.post.imageUrl} alt="" />
      ) : (
        ""
      )}
      ...
      <h4
        className="post__text font-gray"
      ...
      {!isLoading ? (
        <>
          <h4 className="post__text font-gray">
            <strong>{post.username}</strong> {post.post.caption}
          </h4>
          {/* Post timestamp */}
          <p className="post__timestamp">
            {moment(post.post.timestamp.toDate().toString()).fromNow()}
          </p>
        </>
      ) : (
        ""
